...
import { useBrowseBottomN } from "./hooks/useBrowseBottomN";
...
export const DropdownMenuGroup = React.memo(({
  ...
  const browseBottomN = useBrowseBottomN();

  const handleLookAtClick = (dropdownOptions) => {
    if (dropdownOptions.topN) {
      browseTopN(menuGroup.id, dropdownOptions.topN);
    }

    if (dropdownOptions.bottomN) {
      browseBottomN(menuGroup.id, dropdownOptions.bottomN);
    }
